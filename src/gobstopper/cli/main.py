...
def load_config_file(config_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if CLICK_AVAILABLE:
                        click.echo(
                            f"⚠️  Warning: tomli/tomllib not available for TOML support",
                            err=True,
                        )
                    return {}
            return config_data
    ...
    if not config_data and CLICK_AVAILABLE:
        click.echo(
            f"⚠️  Warning: Config file '{config_name}.json' or '{config_name}.toml' not found",
            err=True,
        )
...
if not CLICK_AVAILABLE:
    ...
    def cli():
        raise ImportError(
            "Click is required for the Gobstopper CLI. "
            "Install with: uv add click (or install gobstopper[cli]). "
            "Run commands as: gobstopper run app:app --dev or python -m gobstopper run app:app --dev"
        )
else:
    ...
    @click.option(
        "--features", "-f", multiple=True, help="Additional features to include"
    )
    ...
    @cli.command()
    @click.option(
        "--categories", "-c", multiple=True, help="Task categories to run workers for"
    )
    ...
    @click.option(
        "--threads", "-t", default=None, type=int, help="Number of threads per worker"
    )
    @click.option(
        "--config",
        "-c",
    ...
    @click.option(
        "--loop",
        default=None,
        type=click.Choice(["auto", "asyncio", "uvloop", "winloop"]),
    ...
    @click.option(
        "--access-log/--no-access-log", default=None, help="Enable/disable access log"
    )
    @click.option(
        "--share",
        is_flag=True,
        help="Enable Flash Preview (share via local network/QR)",
    )
    @click.option(
        "--metrics/--no-metrics",
        default=None,
        help="Enable Prometheus metrics exporter",
    )
    @click.option(
        "--metrics-port", type=int, default=9090, help="Metrics exporter port"
    )
    @click.option(
        "--backpressure",
        type=int,
    ...
    @click.option(
        ...
        type=int,
        default=None,
        help="Maximum memory (MiB) before worker respawn",
    )
    @click.option(
        "--lifetime", default=None, help='Maximum worker lifetime (e.g., "1h", "24h")'
    )
    ...
    @click.option(
        ...
        "-d",
        is_flag=True,
        help="Enable Development Mode (implies reload, metrics, debug log)",
    )
    def run(
        app: str,
        host: Optional[str],
        ...
        if dev:
            ...
            if workers is None:
                workers = 1
            click.echo(
                "🛠️  Development mode active: Enabling hot-reload, debug logs, and metrics."
            )
        ...
        if loop in (None, "auto"):
            if is_windows:
                loop = "winloop"
                click.echo(
                    "🪟 Detected Windows environment, using winloop for best performance."
                )
            else:
                loop = "uvloop"
        ...
        if loop == "uvloop":
            try:
                ...
                click.echo(
                    "⚠️  uvloop is not installed. Falling back to asyncio.", err=True
                )
            ...
            try:
                ...
                click.echo(
                    "⚠️  winloop is not installed. Falling back to asyncio.", err=True
                )
                click.echo(
                    "💡 Tip: run 'uv add winloop' for better performance on Windows."
                )
                loop = "asyncio"
        ...
        if not runtime_mode:
            if machine in ("arm64", "aarch64"):
                ...
                click.echo(
                    f"🍎 Detected ARM architecture ({machine}), using single-threaded mode"
                )
            else:
                runtime_mode = "mt"  # Multi-threaded for x86_64
                click.echo(
                    f"💻 Detected x86_64 architecture, using multi-threaded mode"
                )
        else:
        ...
        if metrics:
            ...
            if g_ver and v_tuple(g_ver) < v_tuple("2.7.0"):
                click.echo(
                    f"⚠️  Metrics requires Granian 2.7.0+, but {g_ver} is installed. Disabling metrics."
                )
                metrics = False
            else:
        ...
        if static_dir.exists() and static_dir.is_dir():
            cmd.extend(
                ["--static-path-mount", "static", "--static-path-route", "/static"]
            )
        ...
        if reload:
            ...
            if reload_paths:
                ...
                for path in reload_paths:
                    ...
                    cmd.append(path)
                click.echo(
                    f"👁️  Smart Watcher active: monitoring {', '.join(reload_paths)}"
                )
        ...
        try:
            ...
            if not uds:
                ...
                while time.monotonic() < deadline:
                    ...
                    if proc.poll() is not None:
                        click.echo(
                            f"❌ Granian exited during startup (code {proc.returncode})",
                            err=True,
                        )
            ...
            click.echo(
                "❌ Error: granian not found. Install with: uv add granian", err=True
            )
            sys.exit(1)

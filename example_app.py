...
attach_openapi(
    app,
    title="Gobstopper Demo",
    version="0.1.0",
    description="Example Gobstopper demo app with OpenAPI docs",
)
...
async def login_page(request: Request):
    ...
    return await app.render_template(
        "login.html", csrf_token=csrf_token, page_title="Login"
    )
...
async def demo_notification_view(request: Request):
    ...
    return jsonify(
        {
            "notifications": [
                {"category": cat, "message": msg} for cat, msg in notifications
            ]
        }
    )
...
async def demo_upload(request: Request):
    ...
    notification(
        request, f"Uploaded {len(uploaded_files)} file(s) successfully!", "success"
    )
...
async def index(request: Request):
    ...
    rust_templates = (
        hasattr(app.template_engine, "using_rust") and app.template_engine.using_rust
    )
    ...
    return await app.render_template(
        ...
        template_engine_info={
            ...
            "hot_reload": rust_templates
            and hasattr(app.template_engine, "enable_hot_reload"),
        ...
        features=[
            ...
            "🦀 Rust-powered routing & static files"
            if rust_static
            else "📁 Static file serving",
            "🦀 Rust template engine with streaming"
            if rust_templates
            else "📄 Jinja2 templating with async",
...
async def dashboard(request: Request):
    ...
    dashboard_data = {
        "metrics": [
            ...
            },
            {
                ...
                else "up",  # Prefer lower error rates
            },
            {
                ...
                else "up",  # Prefer lower response times
            },
            {
                "name": "Uptime",
    ...
    request.logger.info(
        f"📊 Generated {len(dashboard_data['metrics'])} metrics, "
    ...
    try:
        ...
        total_data_points = (
            len(dashboard_data["metrics"])
            + len(dashboard_data["recent_activity"])
            + len(dashboard_data["chart_data"])
        )
        ...
        if use_streaming:
            request.logger.info(
                f"🦀 Using Rust STREAMING render for {total_data_points} data points"
            )
            ...
            if hasattr(app.template_engine, "render_template"):
                ...
                result = await app.render_template(
                    "dashboard.html",
                    csrf_token=csrf_token,
                    dashboard_data=dashboard_data,
                    chart_data_json=chart_data_json,
            ...
            request.logger.info(
                f"📄 Using standard render for {total_data_points} data points"
            )
            result = await app.render_template(
                "dashboard.html",
                csrf_token=csrf_token,
        ...
        request.logger.error(
            f"❌ Dashboard template render failed: {type(e).__name__}: {e}",
            exc_info=True,
        )

        # If Rust template fails, fall back to a simple error response
        request.logger.warning(
            "🔄 Attempting to render simple error template instead..."
        )
        try:
            return await app.render_template(
                "error.html",
...
@request_body(
    model=CreateUserBody, media_types=["application/json", "application/x.msgpack"]
)
...
async def get_user(request: Request, user_id: str):
    ...
    user_data = {
        ...
        "email": last_created.get("email")
        if just_created
        else f"user{user_id}@example.com",
        "created_at": last_created.get("created_at")
        if just_created
...
async def queue_processing_task(request: Request):
    ...
    task_id = await app.add_background_task(
        "process_data", "processing", TaskPriority.NORMAL, data=items
    )
...
async def list_tasks(request: Request):
    ...
    return jsonify(
        {"stats": stats, "message": "Task statistics retrieved successfully"}
    )
